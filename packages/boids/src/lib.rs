use bevy::{
    ...
    prelude::{App, Assets, Component, Handle, Res, Transform},
    render2::{
        camera::*,
        color::Color,
        mesh::{shape, Mesh},
...
fn boids_flocking_system(
    ...
    query.for_each_mut(|(mut boid, transform_a, velocity_a, entity_a, flock_a)| {
        ...
        inner_query.for_each(|(transform_b, velocity_b, entity_b, flock_b)| {
            ...
            if percieve(pos_a, dir_a, pos_b, config.neighbor_radius, config.field_of_vision) {
        ...
        if foreign_count > 0 {
            ...
            acceleration +=
                steer(velocity_a, avg_interflock_avoidance, config.max_velocity, config.max_acceleration) * config.interflock_separation_force;
        }
        ...
        if flockmate_count > 0 {
            ...
            acceleration += steer(velocity_a, avg_avoidance, config.max_velocity, config.max_acceleration) * config.separation_force;
            acceleration += steer(velocity_a, offset_to_avg_pos, config.max_velocity, config.max_acceleration) * config.cohesion_force;
            acceleration += steer(velocity_a, avg_heading, config.max_velocity, config.max_acceleration) * config.align_force;
        }
        let target_pos = Vec2::ZERO;
        acceleration += steer(velocity_a, target_pos - pos_a, config.max_velocity, config.max_acceleration) * config.target_force;
        ...
        if raycast(raycastable.iter(), pos_a, dir_a * config.collision_radius) {
            ...
            acceleration += steer(velocity_a, direction, config.max_velocity, config.max_acceleration) * config.collision_avoidance_force;
        }
...
// Set up a scene
fn setup(mut commands: Commands, config: Res<BoidConfig>, mut meshes: ResMut<Assets<Mesh>>) {
    commands
    ...
    commands
        .spawn_bundle(PbrBundle {
            ...
            material: config.materials[0].clone(),
